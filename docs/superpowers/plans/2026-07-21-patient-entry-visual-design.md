...
- **No opacity-modifier Tailwind classes on token colors.** Tokens are stored as literal hex in CSS custom properties (`--primary: #DB6591;`), not HSL triples, so Tailwind utilities like `bg-primary/20` will NOT apply alpha correctly. Wherever a tinted/translucent version of a token is needed, use an explicit 8-digit hex arbitrary value instead, e.g. `bg-[#EE908D33]` (`33` = ~20% alpha, `4D` = ~30%, `1A` = ~10%). This applies to every task in this plan and any follow-up plan reusing these tokens.
